...
-(void)drawTheRoundPoint:(CGPoint)point index:(NSInteger)index Value:(CGFloat)value
{
    ...
    UIImageView *imgAngle = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 6, 6)];
    imgAngle.image = [UIImage imageNamed:@"img_triangle_blue"];
    //[SBImageView getWithFrame:CGRectMake(0, 0, 6, 6) image:IMG_Name(@"img_triangle_blue")];
...
-(void)tapScrollView:(UITapGestureRecognizer *)tapView
{
    ...
    [arrPoint enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (point.x > tapX - GapBetweenX/2 && point.x < tapX + GapBetweenX/2) {
            ...
            lb.hidden = NO ;
            imgAngle.hidden = NO ;
            img.image = [UIImage imageNamed:@"img_point_blue"];
